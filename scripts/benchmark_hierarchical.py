...
from atlas.hierarchical.benchmark import HierarchicalBenchmark, print_benchmark_results
...
def main():
    ...
    parser.add_argument(
        "--depth", type=int, default=1, choices=[0, 1, 2, 3], help="Maximum tree depth (default: 1)"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.2, help="Expansion threshold (default: 0.2)"
    )
    parser.add_argument(
        "--samples", type=int, default=100, help="Number of samples to benchmark (default: 100)"
    )
    parser.add_argument("--top-k", type=int, default=3, help="Top-K paths for decode (default: 3)")
    ...
    if args.operation == "encode":
        results = benchmark.benchmark_encode(
            texts, max_depth=args.depth, expand_threshold=args.threshold
        )
        ...
        results = benchmark.benchmark_decode(trees, top_k=args.top_k, with_reasoning=True)
    else:  # roundtrip
        results = benchmark.benchmark_roundtrip(
            texts, max_depth=args.depth, expand_threshold=args.threshold, top_k=args.top_k
        )
