...
def _budgets(tokens=8000, nodes=8, edges=0, time_ms=50) -> Budgets:
    ...
    return cast(Budgets, {"tokens": tokens, "nodes": nodes, "edges": edges, "time_ms": time_ms})


class _BoomShim:
    @staticmethod
    def select_topk_for_stream(_z, _k, _rng=None):
        raise RuntimeError("boom")

    @staticmethod
    def select_topk_for_global(_z, _k, _exclude_ids=None, _rng=None):
        return []
...
def test_cb_forces_fallback_for_n_ticks(monkeypatch):
    ...
    monkeypatch.setattr(core, "ZSpaceShim", _BoomShim, raising=True)
    fab = FABCore(
        selector="z-space", session_id="cb2", ab_shadow_enabled=False, z_cb_cooldown_ticks=3
    )
...
def test_timeout_triggers_cb():
    fab = FABCore(
        selector="z-space", session_id="cb3", ab_shadow_enabled=False, z_time_limit_ms=0.0001
    )
...
def test_cb_reason_timeout():
    ...
    fab = FABCore(
        selector="z-space",
        session_id="cb-reason-1",
        ab_shadow_enabled=False,
        z_time_limit_ms=0.0001,
    )
    fab.init_tick(mode="FAB0", budgets=_budgets(time_ms=0))
    fab.fill(_make_z(time_ms=0))
    d = fab.mix()["diagnostics"]["derived"]
...
def test_cb_reason_counts_accumulate(monkeypatch):
    ...
    monkeypatch.setattr(core, "ZSpaceShim", _BoomShim, raising=True)
    fab = FABCore(
        selector="z-space", session_id="cb-accum", ab_shadow_enabled=False, z_cb_cooldown_ticks=2
    )
