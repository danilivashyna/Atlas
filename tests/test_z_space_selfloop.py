...
def test_selfloop_pushes_aggressive_when_presence_high():
    ...
    # Check EMA updated
    assert fab.self_presence_ema >= 0.8, f"Expected self_presence_ema >= 0.8, got {fab.self_presence_ema}"
...
def test_selfloop_pushes_conservative_when_presence_low():
    ...
    assert fab.self_presence_ema <= 0.3, f"Expected self_presence_ema <= 0.3, got {fab.self_presence_ema}"
