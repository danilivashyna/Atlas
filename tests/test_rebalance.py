...
def test_rebalance_coverage_penalty_close():
    ...
    config = RebalanceConfig(distance_threshold=0.3, min_distance_boost=0.1, max_penalty=0.5)
    rebalancer = MMRRebalancer(config=config)
...
def test_rebalance_score_pure_diversity():
    ...
    config = RebalanceConfig(lambda_weight=0.0, distance_threshold=0.3, max_penalty=0.5)
    rebalancer = MMRRebalancer(config=config)
...
def test_rebalance_score_balanced():
    ...
    config = RebalanceConfig(
        lambda_weight=0.5, distance_threshold=0.5, min_distance_boost=0.1, max_penalty=0.4
    )
    rebalancer = MMRRebalancer(config=config)
...
def test_rebalance_batch_greedy_selection():
    ...
    config = RebalanceConfig(lambda_weight=0.5, distance_threshold=0.3, max_penalty=0.5)
    rebalancer = MMRRebalancer(config=config)
...
def test_rebalance_realistic_scenario():
    ...
    config = RebalanceConfig(
        lambda_weight=0.6, distance_threshold=0.4, max_penalty=0.3  # Favor relevance slightly
    )
    rebalancer = MMRRebalancer(config=config)
