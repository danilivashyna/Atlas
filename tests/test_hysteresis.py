...
def test_hysteresis_dead_band_prevents_dithering():
    ...
    state = HysteresisState(
        current_precision="mxfp5.25",
        last_change_tick=50  # Changed 50 ticks ago (>rate_limit)
    )
...
def test_hysteresis_realistic_scenario():
    ...
    config = HysteresisConfig(
        ...
        hot_up=0.85, hot_down=0.75,
        warm_high_up=0.65, warm_high_down=0.55
    )
    ...
    for score in scores:
        final_precision, state = assign_precision_hysteresis(score, state, config, current_tick=tick)
        tick += 1
