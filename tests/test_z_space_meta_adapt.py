...
def test_meta_tighten_on_stable_fast(monkeypatch):
    ...
    assert diag["z_meta_last_decision"] in ("tighten", "hold", "loosen"), f"Invalid decision: {diag['z_meta_last_decision']}"
    ...
    assert min_bound <= final_target <= max_bound, f"Target {final_target} out of bounds [{min_bound}, {max_bound}]"
...
def test_meta_loosen_on_unstable(monkeypatch):
    ...
    # Verify meta-learning triggered
    assert diag["z_meta_enabled"] is True
    assert diag["z_meta_last_decision"] in ("loosen", "hold"), f"Expected loosen/hold, got {diag['z_meta_last_decision']}"
    ...
    if diag["z_meta_last_decision"] == "loosen":
        assert final_target > initial_target, f"Target should increase: {initial_target} → {final_target}"
