...
def make_cluster_nodes(cluster_id: int, center_score: float, num_nodes: int, spread: float = 0.05):
    ...
    return [
        {"id": f"c{cluster_id}_n{i}", "score": max(0.0, min(1.0, center_score + (i - num_nodes/2) * spread / num_nodes))}
...
def test_hysteresis_api_cycle_rollout():
    ...
    for envelope_mode, expected_immediate_upgrade in [("legacy", True), ("hysteresis", False)]:
        ...
        for tick in range(3):
            ...
            response = client.post("/api/v1/fab/context/push", json={
                "mode": "FAB0",
...
def test_hysteresis_prevents_oscillation():
    ...
    assert hysteresis_changes < legacy_changes, \
...
def test_tiny_stream_guard_prevents_upgrade():
    ...
    assert ctx_hys["stream_precision"] == "mxfp4.12", \
    ...
    assert precision_level(ctx_hys_down["stream_precision"]) <= precision_level("mxfp8.0"), \
...
def test_diversity_sanity_balanced_clusters():
    ...
    assert cluster_a_count >= 3, \
        f"Expected ≥3 cluster A nodes (relevance), got {cluster_a_count}"
    assert cluster_b_count >= 3, \
        f"Expected ≥3 cluster B nodes (diversity), got {cluster_b_count}"
    
    # Total should equal stream cap
    assert cluster_a_count + cluster_b_count == 16, \
...
def test_changes_per_1k_float_precision():
    ...
    assert 200.0 <= changes_per_1k <= 500.0, \
        f"Expected ~300.0 changes_per_1k, got {changes_per_1k}"
...
def test_chronic_tiny_stream_budget():
    ...
    assert ctx_legacy["stream_precision"] == "mxfp8.0", \
    ...
    assert ctx_hys_2["stream_precision"] == initial_precision, \
    ...
    assert precision_level(ctx_hys_3["stream_precision"]) <= precision_level(initial_precision), \
...
def test_diversity_metric_observability():
    ...
    assert diversity_tight < diversity_diverse, \
...
def test_session_seed_caching():
    ...
    assert fab1.session_seed == fab2.session_seed, \
    ...
    assert fab3.session_seed != fab1.session_seed, \
        "Different session_id should produce different session_seed"
    ...
    assert stream1_ids == stream2_ids, \
        "Cached session_seed should produce deterministic results"
...
def test_unknown_precision_safe_hold():
    ...
    assert ctx["stream_precision"] in ["mxfp4.12", "mxfp5.25", "mxfp6.0", "mxfp8.0"], \
    ...
    assert not (precision_level("unknown") > precision_level("mxfp4.12")), \
...
def test_runtime_envelope_mode_switch(tmp_path):
    ...
    for tick in range(3):
        ...
        if tick == 0:
            ...
            assert precision_level(initial_precision) >= precision_level("mxfp6.0"), \
    ...
    assert ctx_final["stream_precision"] in ["mxfp4.12", "mxfp5.25", "mxfp6.0", "mxfp8.0"], \
...
def test_immediate_downgrade_with_rate_limit():
    ...
    assert ctx_blocked["stream_precision"] == downgraded_precision or \
           ctx_blocked["stream_precision"] == "mxfp4.12", \
...
def test_seeding_discipline_across_budgets():
    ...
    for budget_size in budget_sizes:
        ...
        assert len(stream_ids) <= max_stream, \
    ...
    assert ctx_a["diagnostics"]["derived"]["selected_diversity"] == \
