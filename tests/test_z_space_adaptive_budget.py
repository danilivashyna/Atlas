...
def test_adapt_increase_on_success(monkeypatch):
    ...
    assert limit_after_1 == pytest.approx(
        initial_limit + 0.25, abs=0.01
    ), f"Expected {initial_limit + 0.25}, got {limit_after_1}"
    assert limit_after_2 == pytest.approx(
        initial_limit + 0.50, abs=0.01
    ), f"Expected {initial_limit + 0.50}, got {limit_after_2}"
    assert limit_after_3 == pytest.approx(
        initial_limit + 0.75, abs=0.01
    ), f"Expected {initial_limit + 0.75}, got {limit_after_3}"
