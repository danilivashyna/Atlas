...
def fab_clean():
    ...
    def _mk(**kw: Any):
        ...
        if "z_meta_target_bounds" in kw:
            typed_args["z_meta_target_bounds"] = as_bounds_tuple(
                kw["z_meta_target_bounds"], default=(1.0, 8.0)
            )
        if "reward_weights" in kw:
            typed_args["reward_weights"] = as_weights4(
                kw["reward_weights"], default=(1.0, -1.0, -1.0, -1.0)
            )
