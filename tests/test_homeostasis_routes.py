...
class MockAuditLogger:
    ...
    def log_decision_made(
        ...
        self.events.append(
            {"event_type": "DECISION_MADE", "run_id": run_id, "policy": policy}
        )
...
class TestAuditQuery:
    ...
    def test_audit_query_by_event_type(self, client: TestClient) -> None:
        ...
        response = client.get(
            "/api/v1/homeostasis/audit?event_type=POLICY_TRIGGERED"
        )
        
        assert response.status_code == 200
        data = response.json()
        assert all(
            item.get("event_type") == "POLICY_TRIGGERED" for item in data["items"]
        )
...
class TestErrorCases:
    ...
    def test_missing_policy_engine(self, app: FastAPI, client: TestClient) -> None:
        ...
        response = client.post(
            "/api/v1/homeostasis/policies/test", json={"metrics": {}}
        )
