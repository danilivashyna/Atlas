...
def _make_core(**overrides: Any):
    ...
    for k, v in overrides.items():
        ...
        elif k in ("ab_shadow_enabled", "z_adapt_enabled", "z_meta_enabled", 
                   "policy_enabled", "reward_enabled", "selfloop_enabled"):
            typed_args[k] = as_bool(v)
        elif k in ("hold_ms", "hysteresis_dwell", "hysteresis_rate_limit", "min_stream_for_upgrade",
                   "z_cb_cooldown_ticks", "z_meta_min_window", "policy_dwell_ticks", "reward_window"):
            typed_args[k] = as_int(v)
        elif k in ("ab_ratio", "z_time_limit_ms", "z_target_latency_ms", "z_limit_min_ms", "z_limit_max_ms",
