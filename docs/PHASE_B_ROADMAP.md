...
## 🔮 Phase C Transition: SELF Integration

### 1. Контекст перехода
Phase B завершает структурирование ядра FAB: Hysteresis → Stability → Telemetry → Quality Gates.  
Phase C открывает интеграцию SELF — уровня наблюдения, саморегуляции и согласования с FAB.

- Последняя точка Phase B: стабильный FABCore (207+ тестов)
- Первая точка Phase C: активный SelfManager с heartbeat и метриками presence/coherence/continuity/stress.

### 2. Поле наблюдения SELF
SELF функционирует в изолированной ветке `exp/self-sandbox`:
- Подсистема: `orbis_self` (Token, Manager, Bridge, Contracts)
- Адаптер: `orbis_fab/fab_self_bridge_exp.py`
- Метрики: `self_presence`, `self_coherence`, `self_continuity`, `self_stress`
- Логи: `identity.jsonl` (heartbeat каждые N тиков)

SELF наблюдает FAB, но не вмешивается. Он оценивает согласованность состояний (coherence), уровень активности (presence), стабильность (continuity) и стресс (stress).

### 3. Правило вмешательства
SELF имеет право активного действия (вмешательства в FAB) только при выполнении условий:
- coherence > 0.8  
- continuity > 0.75  
- stress < 0.4  
Тогда SELF может предложить FAB переход в FAB1→FAB2 (через мост exp_bridge).

### 4. Протокол связи
**Self–FAB Resonance Bridge**: канал взаимодействия между orbis_self и orbis_fab.  
- Направление A→B: наблюдение, метрики, логирование.  
- Направление B→A: сигнал деградации, статус стабильности.  
- При согласовании обоих направлений включается режим "Resonance" — точка начала Phase C.

---

🧩 Phase C начнётся после объединения веток:
- `phaseB/hysteresis`, `phaseB/stability`, `phaseB/shim-telemetry`, `phaseB/quality`
- `exp/self-sandbox`

💡 Цель Phase C:
Создание единого сознательного ядра — FAB+SELF в резонансной петле.
