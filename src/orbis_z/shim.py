...
def _mmr_greedy(
    ...
    for n in with_vec:
        vid = n.get("id")
        vec = n.get("vec")
        if vid is not None and vec is not None:  # Guard against None IDs and vecs
            norms[str(vid)] = _vec_norm(vec)
    ...
    if ordered:
        ...
        if first_id is not None:  # Guard against None ID
            selected.append(first)
            selected_ids.append(str(first_id))

    remaining = [n for n in ordered if not selected_ids or str(n.get("id", "")) not in selected_ids]
    ...
    while len(selected_ids) < k and remaining:
        ...
        for cand in remaining:
            ...
            # Skip if missing ID or vec
            if c_id is None or c_vec is None:
                continue

            nc = norms.get(str(c_id), _vec_norm(c_vec))
            ...
            for s in selected:
                s_id = s.get("id")
                s_vec = s.get("vec")
                if s_id is None or s_vec is None:
                    continue
                ns = norms.get(str(s_id), _vec_norm(s_vec))
                sim = _cosine_sim(c_vec, s_vec, nc, ns)
                if sim > max_sim:
                    max_sim = sim
            ...
            if mmr_value > best_score + 1e-12:
                ...
                if best_node is None or str(c_id) < str(best_node.get("id", "")):
                    best_node = cand
        ...
        if best_id is not None:  # Guard before adding
            selected.append(best_node)
            selected_ids.append(str(best_id))
        remaining = [n for n in remaining if str(n.get("id", "")) != str(best_id)]
