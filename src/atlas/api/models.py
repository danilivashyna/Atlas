...
class EncodeRequest(BaseModel):
    ...
    lang: Optional[str] = Field(
        None, pattern="^(ru|en)$", description="Language hint: 'ru' or 'en'"
    )
...
class DecodeRequest(BaseModel):
    ...
    vector: List[float] = Field(
        ..., min_length=5, max_length=5, description="5D semantic vector to decode"
    )
    ...
    def validate_vector(cls, v):
        ...
        if any(abs(x) > 1.0 for x in v):
            pass  # Just a warning, we'll clip values
        return v
...
class DecodeResponse(BaseModel):
    ...
    reasoning: List[DimensionReasoning] = Field(
        default_factory=list, description="Dimension reasoning"
    )
        ...
        json_schema_extra={
            "example": {
                ...
                "reasoning": [
                    ...
                    {"dim": 4, "weight": 0.8, "label": "домашнее", "evidence": ["дома", "хозяин"]},
                ],
                "explainable": True,
...
class DimensionExplanation(BaseModel):
    ...
    examples: List[str] = Field(
        default_factory=list, description="Example words for this dimension"
    )
...
class ErrorResponse(BaseModel):
    ...
    trace_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Request trace ID for debugging"
    )
    timestamp: str = Field(
        default_factory=lambda: datetime.now(timezone.utc)
        .isoformat(timespec="milliseconds")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "trace_id": "req_err123",
                "timestamp": "2025-01-19T12:34:56.789Z",
                "debug_info": {"input_length": 0, "expected_length": "> 0"},
            }
        }
    )
...
class MetricsResponse(BaseModel):
    ...
    requests_by_endpoint: Dict[str, int] = Field(
        default_factory=dict, description="Requests per endpoint"
    )
