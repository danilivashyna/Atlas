...
def print_benchmark_results(results: Dict[str, Any], verbose: bool = True):
    ...
    if "tree_stats" in results:
        ...
        if verbose:
            ...
            print(
                f"  Nodes Range:    [{tree_stats['num_nodes']['min']}, {tree_stats['num_nodes']['max']}]"
            )
            print(
                f"  Branches Range: [{tree_stats['num_branches']['min']}, {tree_stats['num_branches']['max']}]"
            )
...
if __name__ == "__main__":
    ...
    # Sample texts
    texts = ["Собака", "Любовь", "Машина", "Радость", "Страх"] * 20  # 100 samples
