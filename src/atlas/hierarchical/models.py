...
class TreeNode(BaseModel):
    ...
    value: List[float] = Field(
        ...
        min_length=5, 
        max_length=5,
        description="5D semantic vector for this node level"
    )
    label: Optional[str] = Field(
        None,
        description="Human-readable label for this node (e.g., 'coarse', 'agent/action')"
    )
    children: Optional[List["TreeNode"]] = Field(
        None,
        max_length=5,
        description="Optional list of 5 child nodes for fine-grained semantics"
    )
    key: Optional[str] = Field(
        None,
        description="Dimension key (e.g., 'dim1', 'dim2.3')"
    )
    weight: Optional[float] = Field(
        None,
        ge=0.0,
        le=1.0,
        description="Router weight/confidence for this branch (0-1)"
    )
...
class EncodeHierarchicalRequest(BaseModel):
    ...
    expand_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Threshold for lazy expansion (router confidence)"
    )
...
class PathReasoning(BaseModel):
    ...
    evidence: List[str] = Field(
        ...
        description="Supporting evidence (words/features)"
    )
...
class DecodeHierarchicalResponse(BaseModel):
    ...
    reasoning: List[PathReasoning] = Field(
        default_factory=list,
        description="Path-wise reasoning contributions"
    )
...
class PathEdit(BaseModel):
    ...
    value: List[float] = Field(
        ...
        min_length=5,
        max_length=5,
        description="New 5D value for this node"
    )
