...
class SemanticSpace:
    ...
    def interpolate(self, text1: str, text2: str, steps: int = 5) -> List[Dict]:
        ...
        for i in range(steps):
            ...
            results.append({
                'step': i,
                'alpha': alpha,
                'vector': interpolated.tolist(),
                'decoded': decoded
            })
        
        return results
    
    def explore_dimension(self, base_text: str, dimension: int, range_vals: List[float]) -> List[Dict]:
        """
    ...
    def visualize_space_2d(self, texts: List[str], dimensions: tuple = (0, 1)) -> plt.Figure:
        ...
        for i, text in enumerate(texts):
            ax.annotate(text, (x[i], y[i]), xytext=(5, 5), 
        ...
        ax.set_xlabel(f"Dimension {dim1+1}: {info1.name} ({info1.poles[0]} ↔ {info1.poles[1]})", 
                     fontsize=11)
        ax.set_ylabel(f"Dimension {dim2+1}: {info2.name} ({info2.poles[0]} ↔ {info2.poles[1]})", 
                     fontsize=11)
