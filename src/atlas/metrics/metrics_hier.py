...
def h_coherence_stub(
    ...
    return CoherenceResult(score=round(score, 3), method=f"{method} (stub - MVP)", status="stub")
...
def h_stability_stub(
    ...
    if not labels_run1 or not labels_run2:
        return StabilityResult(score=0.0, method=f"{method} (stub)", status="stub")
    ...
    return StabilityResult(score=round(score, 3), method=f"{method} (stub - MVP)", status="stub")


def interpretability_metrics_summary(
    coherence: Optional[CoherenceResult] = None, stability: Optional[StabilityResult] = None
) -> Dict[str, Any]:
    """
