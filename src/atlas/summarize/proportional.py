...
def collect_evidence_per_dimension(
    text: str, vector: np.ndarray, dimension_labels: Optional[List[str]] = None
...
def calculate_token_quotas(distribution: np.ndarray, target_tokens: int) -> List[int]:
    """
...
def greedy_fill_by_dimension(
    evidence: Dict[int, List[Tuple[str, float]]], quotas: List[int], encoder=None
    ...
    for round_idx in range(max_rounds):
        ...
        for dim_idx in range(5):
            ...
            available = [
                (text, score) for text, score in evidence[dim_idx] if text not in used_pieces
            ]
    ...
    for iteration in range(max_iterations):
        ...
        try:
            ...
            if current_dist[worst_dim] < target_distribution[worst_dim]:
                ...
                available = [text for text, score in evidence[worst_dim] if text not in used_set]
...
def summarize(
    ...
    selected_pieces, actual_tokens = greedy_fill_by_dimension(evidence, quotas, encoder)
